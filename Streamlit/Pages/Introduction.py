...
def prediction2(ticker, window_size=20, forecast_days=14):
    ...
    start_date = end_date - pd.Timedelta(days= forecast_days + 365 + window_size)  # On veut voir la courbe sur forecast_days, mais faut les windows_size dernières données 
    df = yf.download(ticker, start=start_date, end=end_date).dropna()
    ...
    real_value_before = yf.download(ticker, start=df.index[0] + pd.Timedelta(days=14),
                             end=df.index[0] + pd.Timedelta(days=15))['Close'].values[0]
    real_value_after = yf.download(ticker, start=df.index[0],
                             end=df.index[0] + pd.Timedelta(days=1))['Close'].values[0]
    
    return predictions_real, real_value_before, real_value_after
