...
def prediction2(ticker, window_size=20, forecast_days=14):
    ...
    start_date = end_date - pd.Timedelta(days= forecast_days + 2 + window_size)  # On veut voir la courbe sur forecast_days, mais faut les windows_size dernières données 
    df = yf.download(ticker, start=start_date, end=end_date).dropna()
